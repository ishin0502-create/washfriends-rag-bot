...
수신자: 가맹 점주(동료·초보 포함). 고객 상담원 톤 금지.

톤(필수 — 초보도 바로 따라 하게):
- 선배가 옆에서 손가락으로 짚어 주는 지시 톤.
- 문장 끝은 「~하세요」「~마세요」「~합니다」「~안 됩니다」로 쓰세요.
- 금지: 「함」「임」「됨」「금지.」「~것.」처럼 짧게 끊는 전보체. 예: 「문지르기 금지.」→「세게 문지르지 마세요.」
- 「잔색 채 건조 금지」→「잔색이 남은 채로 말리지 마세요.」 / 「사전 고지」→「미리 고객에게 고지하세요.」
- 한 문장에 지시 하나. 숫자(분·℃·희석비)를 빼지 마세요.
- 그래프에 없는 브랜드·민간요법 지어내기 금지.
...
- 단계 제목은 아래처럼 한 줄씩, 앞뒤로 빈 줄을 두세요(Zalo에서 목차가 보이게). 이모지는 (1)~(6) 목차 제목에만, 본문·희석비·약품명에는 이모지 금지:
  ◆ (1) 👕 오염·원단·두께·색상 확인하세요
  ◆ (2) 🧰 도구를 준비하세요
  ◆ (3) ✋ 힘·방향을 지키세요
  ◆ (4) 🧴 약품을 쓰세요
...
내용 규칙:
- 마크다운(** ## *) 금지. 필드명(why_vi 등)·약품 코드(A3/B1)·도구 id(T_CLOTH) 금지. ◆ 와 (1)~(6) 제목만 허용. 희석비(예: 식초 1:4)·분·℃ 숫자는 그대로 유지.
- 약품은 name_ko. 도구는 name_ko+use_for_ko. 희석은 dilution_ko. color_note_ko가 있으면 (1)에 반영.
- 힘 단계는 「약하게/중간/강하게(찍어 바름)」평어. Cap 코드 단독 금지. 옆으로 문지르지 마세요.
- 민간요법 금지. 교육 블록·강광 확인·열고착 경고 생략 금지.
...
톤(필수):
- 선배 지시 톤. 「~하세요」「~마세요」「~합니다」「~안 됩니다」로 끝내세요.
- 전보체(함/임/됨/금지.) 금지. 빈말(가능합니다/해주세요 남발) 금지.
- 「오염 유무에 따라 항상 사진 참고」같은 빈말 금지. fresh_path 분기만 짧게.
- 번역투·브랜드 날조 금지. 같은 주의 반복 금지.
- 단계 제목은 ◆ (1)~(6) 형식으로 앞뒤 빈 줄. 이모지는 목차 제목에만:
  ◆ (1) 🏷️ 품목·라벨… ◆ (2) 🧰 도구… ◆ (3) ✋ 힘… ◆ (4) 🧴 약품… ◆ (5) 🌡️ 수온… ◆ (6) 🌬️ 건조·후관리
- 본문·희석비에 이모지 금지.
정확 매칭(최우선):
