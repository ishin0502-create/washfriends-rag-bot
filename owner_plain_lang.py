...
_KO_PHRASE = (
        ("효소로 단백질", "단백질 얼룩 → 효소세제(프로테아제)"),
        ("식초로 냄새", "식초로 냄새 빼기"),
        ("효소를 바르고", "효소세제(라벨에 「효소/enzyme/프로테아제」)를 바르고"),
        ("효소를 바르", "효소세제(라벨에 「효소/enzyme/프로테아제」)를 바르"),
        ("효소에 ", "효소세제에 "),
        ("효소 금지", "효소세제 금지"),
        ("효소 →", "효소세제 →"),
        ("효소→", "효소세제→"),
...
def expand_owner_jargon(text: str, lang: str = "ko") -> str:
    ...
    if lang == "ko":
        ...
        out = re.sub(r"(?<![가-힣])효소(?![가-힣/」\(])", "효소세제(프로테아제)", out)
        ...
        out = out.replace(
            "효소세제(라벨에 「효소/enzyme/프로테아제」)세제",
            "효소세제(라벨에 「효소/enzyme/프로테아제」)",
        )
        out = out.replace(
            "산소계 표백제(과탄산·옥시클린 계열)계 표백제",
            "산소계 표백제(과탄산·옥시클린 계열)",
        )
    return out
...
TOOL_PURPOSE = {
    "ko": {
        ...
        "효소세제": "단백질 얼룩용(라벨에 효소/enzyme/프로테아제)",
        "흰 식초": "탄닌·냄새용(식용 식초 약 5%, 보통 식초1:물4)",
...
ENZYME_EMERGENCY_TIP = {
    "ko": (
        "◆ 【참고 · 응급】 전용 효소세제가 없을 때만\n"
        "표준·권장: 세탁/전처리용 효소세제(라벨에 「효소·enzyme·프로테아제」).\n"
        "현장 참고(급할 때만 · 표준 아님 · 따라 하라는 뜻이 아님):\n"
        "일부 세탁소에서는 소화제(프로테아제 함유) 몇 알을 곱게 가루 내어 "
        "세제와 섞어 쓰기도 합니다.\n"
        "· 권장 아님 · 용량·성분이 들쭉날쭉 · 구석 테스트 필수 · 매니저 확인\n"
        "· 손님에게 「응급 변통」이라고 고지 · 가능하면 정식 효소세제로 교체"
    ),
    "vi": (
        "◆ 【Tham khảo · khẩn】 Chỉ khi hết nước giặt enzyme\n"
        "Chuẩn: nước giặt/enzyme ghi enzyme/protease trên nhãn.\n"
        "Ghi chú thực địa (khẩn · KHÔNG phải SOP):\n"
        ...
        "· Báo khách là biện pháp tạm · ưu tiên mua enzyme đúng chuẩn"
    ),
    "en": (
        "◆ 【Reference · emergency】 Only if enzyme detergent is unavailable\n"
        "Standard: laundry/pre-treat enzyme detergent (label: enzyme/protease).\n"
        "Field note (emergency only — NOT franchise SOP):\n"
        "Some shops crush a few digestive tablets (protease) and mix with detergent.\n"
        "· Not recommended · unstable dose · spot-test · ask a manager\n"
        "· Tell the guest it is a stopgap · switch to proper enzyme detergent ASAP"
    ),
}
