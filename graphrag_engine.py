...
def _polish_owner_ko_phrasing(answer: str, *, item_wash: bool = False) -> str:
    ...
    out = re.sub(
        r"「([^」]+)」라고\s*(?:펜으로\s*)?적는다",
        r"「\1」처럼 분무기 겉면 라벨에 적어 두세요",
        out,
    )
    out = re.sub(
        r"(식초\s*1\s*[:：]\s*물\s*4)\s*라고\s*(?:펜으로\s*)?적는다",
        r"\1처럼 분무기 겉면 라벨에 적어 두세요",
        out,
    )
    out = out.replace("이라고 펜으로 적는다", "처럼 분무기 겉면 라벨에 적어 두세요")
    out = out.replace("라고 펜으로 적는다", "처럼 분무기 겉면 라벨에 적어 두세요")
    # Avoid double "분무기 겉면 라벨에 … 분무기 겉면 라벨에"
    out = out.replace("병 겉에 ", "분무기 겉면 라벨에 ")
    out = re.sub(
        r"분무기 겉면 라벨에\s*(「[^」]+」)\s*처럼\s*분무기 겉면 라벨에\s*적어 둔다",
        r"분무기 겉면 라벨에 \1처럼 적어 두세요",
        out,
    )
    replacements = (
        ...
        ("미확인 시 얇음으로 약하게", "미확인 시 약하게(흡수·찍어 바름만)"),
        ("미확인시 얇음으로", "미확인 시"),
        ("미확인 시 얇음으로", "미확인 시"),
        ("얇음으로 약하게(흡수·찍기만)", "약하게(흡수·찍어 바름만)"),
        ("얇음으로 약하게", "약하게(흡수·찍어 바름만)"),
        ("얇음으로,", "보통 두께 기준,"),
        ("약하게(흡수·찍기만)(흡수·찍기만)", "약하게(흡수·찍어 바름만)"),
        ("약하게(흡수·찍기만)", "약하게(흡수·찍어 바름만)"),
        ("흰 식초을", "흰 식초를"),
        ("바깥→안 문지름", "바깥→안 찍어 바름"),
        ("바깥→안으로 문지름", "바깥→안 찍어 바름"),
        ("바깥→안으로 가볍게 문지름", "바깥→안 가볍게 찍어 바름"),
        ("가볍게 문지름", "가볍게 찍어 바름(세게 문지르지 마세요)"),
        ("가볍게 문지른다", "가볍게 찍어 바르세요(세게 문지르지 마세요)"),
        ("가볍게 마사지", "가볍게 찍어 바름"),
        ("바깥→안으로 닦는다", "바깥→안 찍어 흡수하세요"),
        ("바깥→안으로 닦아", "바깥→안 찍어 흡수해"),
        # Telegram endings → instructional
        ("문지르기 금지.", "세게 문지르지 마세요."),
        ("문지르기 금지", "세게 문지르지 마세요"),
        ("절대 금지.", "절대 하지 마세요."),
        ("절대 금지", "절대 하지 마세요"),
    ...
    out = re.sub(r"가벼운\s*힘으로\s*바깥→안(?:으로)?\s*닦는다", "약하게 바깥→안 찍어 흡수하세요", out)
    if "문지르지 마세요" in out or "문지르기 금지" in out:
        out = re.sub(r"바깥→안\s*문지른다", "바깥→안 찍어 바르세요", out)
    ...
    out = out.replace("분무기 겉면 라벨에 라벨 붙임", "분무기 겉면 라벨에 약 이름·희석비를 적어 두세요")
...
def _promote_ko_section_headers(text: str, *, item_wash: bool = False) -> str:
    ...
    for key, title in edu_map.items():
        out = re.sub(
