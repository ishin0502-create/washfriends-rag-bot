...
async def health():
    ...
    return JSONResponse(
        content={
            ...
            "build": "2026-09-22-hair-dye-ipa-fail-v44b",
            "checks": checks,
        },
        status_code=200,
    )
...
UNWIND [
  ...
  {code:'A3',name:'White Vinegar 5%',name_vi:'Giấm trắng 5%',role:'Mild acid breaks tannin bonds neutralizes alkali odor',safe_on_wool:false,safe_on_silk:false,shop_name_vi:'Giấm trắng ăn / giấm tinh',buy_where_vi:'Siêu thị',alt1_vi:'Giấm ăn pha loãng',alt2_vi:'Nước cốt chanh pha (test màu)',alt3_vi:'',example_brands_vi:'',wf_supply:false,when_use_vi:''},
...
UNWIND [
  ...
  {id:'S_SUNSCREEN',name:'Sunscreen',name_vi:'Kem chong nang',name_ko:'선크림·자외선 차단제',group_id:'G2',water_spreads:false,contains_protein:false,contains_tannin:false,contains_oil:true,contains_dye:false,urgency:'same_day',tip:'VN common: UV filter+silicone oil+zinc. N3 oil then D2; NEVER B2 (permanent yellow)'},
  {id:'S_TAR',name:'Tar / asphalt',name_vi:'Nhua duong / tar',name_ko:'타르·아스팔트',group_id:'G2',water_spreads:false,contains_protein:false,contains_tannin:false,contains_oil:true,contains_dye:true,urgency:'same_day',tip:'Hardest oil: freeze scrape then D1 many cycles ventilated — patience'}
] AS s MERGE (n:Stain {id:s.id}) SET n += s RETURN count(n) AS created""")
...
UNWIND [
  ...
  {id:'S_MASCARA',name:'Mascara',name_vi:'Masca / mascara',name_ko:'마스카라',group_id:'G4',water_spreads:false,contains_protein:false,contains_tannin:false,contains_oil:true,contains_dye:true,urgency:'same_day',tip:'Regular: A1 blot; waterproof: D1. Carbon black — never rub. NEVER B2 (permanent gray)'},
  {id:'S_HAIR_DYE',name:'Hair dye',name_vi:'Thuoc nhuom toc',name_ko:'헤어 염색약·염모제',group_id:'G4',water_spreads:true,contains_protein:false,contains_tannin:false,contains_oil:false,contains_dye:true,urgency:'immediate',tip:'Act fast cold rinse; alcohol/oxygen carefully; test fabric; low success if set'}
] AS s MERGE (n:Stain {id:s.id}) SET n += s RETURN count(n) AS created""")
...
""",
        ...
        _r(s, "T_tools", """
UNWIND [
  {id:'T_BRUSH_SOFT',name_vi:'Bàn chải spotting mềm',name_ko:'연질 스포팅 솔',
   use_for_vi:'(runtime) Theo vết/vải/độ dày — không dùng câu mẫu cố định.',
   use_for_ko:'(런타임) 오염·원단·두께에 맞게 다시 씀 — 고정 시드 문장 사용 금지.',
   use_for_en:'(runtime) Rewritten per stain/fabric/weight — do not use fixed seed copy.'},
  {id:'T_BRUSH_HARD',name_vi:'Bàn chải spotting cứng',name_ko:'경질 스포팅 솔',
   use_for_vi:'(runtime) Theo bùn/dầu/denim — không mẫu cố định.',
   use_for_ko:'(런타임) 흙·유성·데님 맥락으로 다시 씀.',
   use_for_en:'(runtime) Rewritten for mud/oil/denim context.'},
  {id:'T_BRUSH_ULTRA',name_vi:'Bàn chải siêu mềm / miếng foam',name_ko:'초연질 솔·스펀지',
   use_for_vi:'(runtime) Theo protein/lụa/len.',
   use_for_ko:'(런타임) 단백질·실크·울 맥락으로 다시 씀.',
   use_for_en:'(runtime) Rewritten for protein/silk/wool.'},
  {id:'T_CLOTH',name_vi:'Khăn trắng sạch / giấy thấm',name_ko:'흰 천·흡수지',
   use_for_vi:'(runtime) Theo tannin/dầu/protein/mực.',
  ...
  {id:'T_BRUSH_SHOE',name_vi:'Bàn chải đế giày (lông cứng)',name_ko:'운동화 밑창용 경질 솔',
   use_for_vi:'Chỉ đế cao su/gai: chải khô bùn rồi chấm nước rửa chén. CẤM thân mesh/lụa.',
   use_for_ko:'고무·클리트 밑창만: 흙 털고 D2. 갑피 메시·실크에 사용 금지.',
   use_for_en:'Rubber/cleat outsole only: dry mud then D2. Never mesh/silk upper.'},
  {id:'T_GLOVE_NITRILE',name_vi:'Găng tay nitrile (bảo hộ)',name_ko:'니트릴 장갑(PPE)',
   use_for_vi:'BẮT BUỘC trước acid oxalic / Javel / cồn / acetone / dung môi / dịch cơ thể. Đeo trước khi mở chai. Không dùng găng mỏng với acid.',
  ...
  {id:'T_MESH_BAG',name_vi:'Túi lưới giặt',name_ko:'세탁망',
   use_for_vi:'(runtime) Rèm/chăn/đồ mỏng — theo item.',
   use_for_ko:'(런타임) 커튼·이불·얇은 옷 품목별로 다시 씀.',
   use_for_en:'(runtime) Rewritten per curtain/duvet/delicate item.'},
  {id:'T_TIMER',name_vi:'Đồng hồ hẹn giờ',name_ko:'타이머',
   use_for_vi:'Đặt hẹn giờ ĐÚNG số phút của quy trình (vd 15-45). Hết giờ → xả ngay. Không để qua đêm.',
   use_for_ko:'이 오염 처리 분(예: 15–45분)을 타이머에 맞추고, 울리면 즉시 헹군다. 밤새 방치 금지.',
   use_for_en:'Set the exact process minutes (e.g. 15-45). When done, rinse immediately. No overnight soak.'},
  {id:'T_SOAK_BIN',name_vi:'Chậu ngâm / thùng ngâm',name_ko:'담금통·침지 용기',
   use_for_vi:'Pha dung dịch, ngâm ĐÚNG số phút (vd 15-45). Dán nhãn. CẤM ngâm suit/cà vạt/lụa mỏng nếu SOP cấm.',
   use_for_ko:'희석액을 통에 만들어 규정 분(예: 15–45분)만 담근다. 정장·넥타이·얇은 실크 통담금은 SOP에서 금하면 하지 말 것.',
   use_for_en:'Mix dilution, soak only the timed minutes (e.g. 15-45). No full-soak for suits/ties/sheer silk if SOP forbids.'},
  {id:'T_UV_LAMP',name_vi:'Đèn UV 365nm (kiểm vết)',name_ko:'UV 램프(잔여 얼룩 검사용)',
   use_for_vi:'Tắt đèn phòng, giữ đèn ~15-20cm, quét tìm vết còn (protein/OBA). Kiểm TRƯỚC sấy.',
  ...
  {id:'T_STEAM_IRON',name_vi:'Bàn ủi hơi',name_ko:'스팀 다리미',
   use_for_vi:'Chỉ SAU khi vết sạch. Đọc nhiệt độ ủi trên NHÃN GIẶT của áo. CẤM ủi khi còn vết (khóa màu). Cà vạt: chỉ xịt hơi ĐỨNG, KHÔNG ép sát bàn ủi.',
...
""",
        ...
        try:
            from chem_owner_vi import chem_seed_overlay_rows as _chem_own
...
UNWIND $rows AS d
MATCH (c:Chemical {code:d.code})
SET c.name_vi = d.name_vi,
    c.shop_name_vi = d.shop_name_vi,
    c.buy_where_vi = d.buy_where_vi,
    c.dilution_vi = d.dilution_vi,
    c.when_use_vi = d.when_use_vi
RETURN count(c) AS updated
""",
                rows=_co,
            )
            log["V12_chem_owner_vi"] = res_co.data()[0] if res_co else {"updated": 0}
        except Exception as e:
            log["V12_chem_owner_vi"] = f"ERR:{str(e)[:120]}"
...
UNWIND [
  {id:'S_BLOOD_FRESH',
   why_vi:'[Tại sao] Mau tuoi = hemoglobin (protein + sat). Nuoc LANH giu protein hoa tan — nuoc nong/say bien tinh → sat bam soi tao vet nau VINH VIEN. Muoi hut mau tuoi; enzyme protease cat chuoi protein; giat sau. Vai trang: oxy gia 3% them (test goc). Len/lua: KHONG enzyme/oxy — S1 trung tinh. CAM meo kem danh rang.',
  ...
  {id:'S_BLOOD_DRY',
   why_vi:'[Tại sao] Mau kho = protein da gan soi. Can enzyme protease pha chuoi lau hon. Nhiet van CAM. Oxy gia 3% chi cotton trang sau test. Bao khach neu vet nau da khoa.',
  ...
  {id:'S_MOTORBIKE_OIL',
   why_vi:'[Tại sao] Dau nhot xe may = dau kho + carbon. Buoc 1 hut (bot ngo/phan rom) → Buoc 2 dung moi tay dau (THONG GIO, khong lua) → Buoc 3 giat. Say khi con nhon = khoa vet. Silk/wool: khong dung moi manh/nhiet cao — bao khach.',
  ...
  {id:'S_BLACK_COFFEE',
   why_vi:'[Tại sao] Ca phe den = tannin + mau. Xu ly SOM nuoc LANH — nhiet/say khoa mau. Giam trang 5% (1:4) pha tannin; con mau: bot tay oxy (khong len/lua). Neu ca phe SUA/latte: enzyme/nuoc rua chen TRUOC roi giam. CAM meo: kem danh rang, ethanol cocktail, giam 1:1.',
  ...
  {id:'S_MILK_COFFEE',
   why_vi:'[Tại sao] Latte/ca phe sua = tannin + protein/mo sua. THU TU BAT BUOC: protein/mo (enzyme protease hoac nuoc rua chen) TRUOC → tannin (giam 1:4) SAU. Dao thu tu = mau khoa. Nhiet cao khoa protein.',
  ...
  {id:'S_FRUIT_JUICE',
   why_vi:'[Tại sao] Juice = tannin + mau hoa qua (anthocyanin/carotenoid). SOM + lanh. Buoc1 giam 1:4. Buoc2 bot tay oxy neu con mau (trang/cotton OK; CAM len/lua). CAM say khoa mau.',
  ...
  {id:'S_KIMCHI',
   why_vi:'[Tại sao] Kim chi/nuoc kim chi = bot ot (mau) + dau + muoi/acid. Xu ly SOM lanh. THU TU: cao bot → xa lanh → nuoc rua chen (dau) → giam 1:4 (mau/mui) → oxy CHI vai trang con mau. CAM say khoa mau ot. CAM meo kem danh rang.',
  ...
  {id:'S_KETCHUP',
   why_vi:'[Tại sao] Ketchup = lycopene (mau ca chua) + duong + mang dau. Cao bot truoc. THU TU: nuoc rua chen (dau) → giam 1:4 (tannin/lycopene) → bot tay oxy neu trang con mau. Nhiet som khoa mau do.',
  ...
  {id:'S_TOMATO_SAUCE',
   why_vi:'[Tại sao] Sot ca chua = lycopene + dau nau. Giong ketchup: cao → nuoc rua chen → giam → oxy trang. Khong cha lan mau do.',
  ...
  {id:'S_MAYO',
   why_vi:'[Tại sao] Mayonnaise = dau (mo) + trung (protein). THU TU: cao → nuoc rua chen/lipase cho DAU TRUOC → enzyme protease cho PROTEIN. Dao thu tu = mo khoa protein. Nhiet cao khoa protein. Khong oxy truoc khi het mo.',
  ...
  {id:'S_COOKING_OIL',
   why_vi:'[Tại sao] Dau an bam soi. Buoc1 hut (bot ngo/phan rom) → Buoc2 nuoc rua chen (surfactant) → Buoc3 enzyme lipase neu co. Say khi con nhon = khoa vet bong. Phan biet dau an vs dau nhot xe may.',
  ...
  {id:'S_GREASE',
   why_vi:'[Tại sao] Mo/grease = lipid dam. Hut bot → nuoc rua chen + lipase. Khong say khi con mo.',
  ...
  {id:'S_SOY_SAUCE',
   why_vi:'[Tại sao] Nuoc tuong = protein len men + tannin/mau den. THU TU: xa lanh → enzyme protease → giam 1:4 → oxy neu trang con mau. Nhiet som khoa mau.',
  ...
  {id:'S_FISH_SAUCE',
   why_vi:'[Tại sao] Nuoc mam = protein + muoi + mau. Mui man kho: giam deodorize. THU TU: xa lanh → enzyme → giam (mau+mui) → oxy trang neu can. Bao khach mui co the ton tai nhe.',
  ...
  {id:'S_COLLAR_STAIN',
   why_vi:'[Tại sao] Vet co ao so mi = sebum + mo hoi + da chet oxy hoa → vang/xam. Enzyme (protease/lipase) len CO KHO truoc khi uot. CAM Javel/chlorine tren protein vang — lam VANG HON (tai lieu laundry cong nghiep). Sau do bot tay oxy. Ban chai mem thoi. CAM ui khi con vet.',
  ...
  {id:'S_SHIRT_YELLOW',
   why_vi:'[Tại sao] Ao so mi trang vang toan than/co/tay = sebum+mo hoi oxy hoa + co the deodorant. KHONG dung chlorine (lam vang hon). THU TU: enzyme TRUOC → bot tay oxy ngam SAU. Soft brush. Lien quan vong co (S_COLLAR) neu chi co.',
  ...
  {id:'S_DYE_TRANSFER',
   why_vi:'[Tại sao] Lo mau / dye transfer = mau tu ao khac bam khi giat. XU LY NGAY — CAM say/ui (nhiet khoa mau). Bot tay oxy ngam dai (toi 6-8 gio theo chai) roi giat lai. Vai TRANG cotton: co the Javel pha loang CAN THAN. Vai mau/len/lua: CHI oxy + test; khong chlorine. Bao khach co the khong het 100%.',
  ...
  {id:'S_STARCH_TRANSFER',
   why_vi:'[Tại sao] Ho tinh bot (starch/풀) + mau lan: starch la mang giu mau. THU TU: enzyme amylase (phan tinh bot) TRUOC → bot tay oxy cho mau SAU. CAM say. Neu chi lo mau khong ho → xu ly nhu S_DYE_TRANSFER.',
  ...
  {id:'S_MILDEW',
   why_vi:'[Tại sao] Nam moc = nam + sac to. An toan: PPE, xu ly NGOAI TROI, chai/hut bot bao tu (CCI). Giam trang giet nam (acid). Bot tay oxy pha sac to. Javel CHI cotton trang + khong tron giam/ammonia. Oxy KHONG phai chat khu trung. CAM say khi con vet moc. Da/suede: chuyen chuyen nghiep.',
  ...
  {id:'S_LIPSTICK',
   why_vi:'[Tại sao] Son moi = 3 lop: (1) sap ngoai — cao nhe, (2) dau giua — con isopropyl THAM mat trai (blot, khong cha), (3) pigment mau — nuoc rua chen roi bot tay oxy neu trang. THU TU BAT BUOC — bo qua sap hoac cha thay vi blot = that bai. Len/lua: test goc; co the chi D2 + S1. CAM say/ui khoa mau. Thong gio khi dung con.',
  ...
  {id:'S_FOUNDATION',
   why_vi:'[Tại sao] Kem nen/cushion = dau nen + pigment. THAM nhe (blot) — KHONG cha lan. Nuoc rua chen cho dau TRUOC → con isopropyl cham cho mau neu can (test goc). Len/lua: chi S1 trung tinh + tham rat nhe. CAM say khi con mau.',
...
UNWIND [
  ...
  {id:'S_CURRY',
   why_vi:'[Tại sao] Ca ri/nghe = dau + curcumin (mau vang kho). THU TU: nuoc rua chen (dau) TRUOC → baking soda/kiềm nhe → anh sang UV/phoi nang co kiem soat (curcumin phai mau) → bot tay oxy neu trang. CAM chi giat nuoc — dau giu mau. Len/lua: test; tranh oxy/nang manh.',
  ...
  {id:'S_INK_PEN',
   why_vi:'[Tại sao] Muc but bi = dye tan trong dung moi. CHI THAM/CHAM mat trai — cha = lan mau. Lot giay tham. Con isopropyl. Het muc moi say (nhiet khoa). Test goc. Len/lua: rat than; co the khong het 100%.',
  ...
  {id:'S_INK_PERMANENT',
   why_vi:'[Tại sao] But long = polymer + pigment. A2 acetone hoac A1 — test vai (co the hong in/son). Thong gio. Khong cam ket 100%. CAM cha lan.',
  ...
  {id:'S_RUST',
   why_vi:'[Tại sao] Ri set = sat oxit. CAM Javel (co dinh sat). Then chot: acid oxalic X2 + gang tay; xa + trung hoa N1. Cotton/linen/poly OK. Len/lua: KHONG X2 — A3/chanh nhe + bao. PPE.',
  ...
  {id:'S_SWEAT_YELLOW',
   why_vi:'[Tại sao] Ve o nach vang = mo hoi + protein + thuong deodorant/aluminum. CAM chlorine (vang hon). THU TU: enzyme (protein) TRUOC → bot tay oxy (vang) SAU. Giam pha neu can khu mui. Ao so mi toan than → xem S_SHIRT_YELLOW.',
...
UNWIND [
  ...
  {id:'S_SUNSCREEN',name_ko:'선크림·자외선 차단제'},
  {id:'S_TAR',name_ko:'타르·아스팔트'},
  {id:'S_MASCARA',name_ko:'마스카라'},
  {id:'S_HAIR_DYE',name_ko:'헤어 염색약·염모제'},
...
SET s.name_ko = '버블티·밀크티·타피오카',
    ...
    s.why_vi = '[Tại sao] Bubble tea = tannin tra + protein sua + tinh bot san + duong. THU TU: xa LANH → enzyme (protein+tinh bot) → D2 (mo sua) → A3 (tannin) → B1 neu trang. Bo enzyme = kem hieu qua.',
...
UNWIND [
  {id:'S_TEA',
   why_vi:'[Tại sao] Nuoc tra (den/xanh/oolong) = tannin + mau. Xu ly SOM + nuoc LANH — nhiet/say khoa mau vang-nau. THU TU: xa lanh → giam trang 1:4 (pha tannin) → bot tay oxy neu trang/cotton con mau. CAM len/lua + B1. Duong trong tra da kho de vang — bao khach.',
  ...
  {id:'S_RED_WINE',
   why_vi:'[Tại sao] Ruou vang do = anthocyanin + tannin + acid/duong. SOM + LANH. Muoi/tham hut bot neu rat tuoi (tuy chon) → giam 1:4 → oxy trang neu can. CAM say khoa mau do-tim. Vai mau: test; khong Javel.',
  ...
  {id:'S_WHITE_WINE_BEER',
   why_vi:'[Tại sao] Ruou vang trang/bia = tannin nhe + duong/men — moi co the GIAN (khong thay) roi VANG sau. Xu ly SOM du khong thay mau. Lanh → giam nhe → giat. Trang: oxy phong vang. CAM say khi nghi con duong.',
  ...
  {id:'S_SOFT_DRINK',
   why_vi:'[Tại sao] Nuoc ngot/cola = duong + mau/acid (cola: caramel mau + phosphoric). Duong kho = dinh + vang. SOM xa lanh → giam neu mau → oxy trang. CAM say khi con ngot/dinh.',
...
UNWIND [
  {id:'S_VOMIT',
   why_vi:'[Tại sao] Chất nôn = acid dạ dày + đạm thức ăn + mật (vàng/xanh) + nguy cơ vi sinh. BẮT BUỘC găng nitrile trước. CẤM nước nóng (khóa đạm). Thứ tự: cạo rắn → xả LẠNH → nước giặt/bột ngâm enzyme (phá đạm) → giấm 1:4 (mùi/acid) → bột tẩy oxy chỉ khi cotton trắng còn màu. Trước khách nói 「vết hữu cơ」, không mô tả chi tiết.',
   fresh_path_vi:'(1) Găng nitrile + khẩu trang nếu mùi nặng; thông gió. (2) Cạo hết phần rắn vào túi — KHÔNG chà. (3) Xả LẠNH mặt trái ngoài→trong. (4) Ngâm enzyme phân giải đạm 30–45 phút (nồng độ theo nhãn, cao hơn bình thường nếu cần). (5) Xả; giấm trắng 1:4 khoảng 15 phút khử mùi/acid. (6) Giặt; cotton trắng còn màu: bột tẩy oxy (test góc). (7) Phơi nắng nếu được. Ánh sáng TRƯỚC sấy.',
   dried_path_vi:'Ngâm enzyme dài/qua đêm → giấm → bột oxy nếu trắng. Báo tỷ lệ thấp nếu đã sấy khóa màu mật.',
   force_metaphor_vi:'Lực vừa: cạo nhẹ + thấm — không chà đẩy acid vào sợi',
   force_metaphor_ko:'중간: 긁어내고 흡수 — 문지르면 위산·잔여물이 섬유로 침투',
   sense_check_vi:'Mắt: hết mảnh thức ăn. Mũi: hết mùi nôn (enzyme/giấm). Tay (găng): vải mềm.',
   sense_check_ko:'눈: 음식물 잔여 없음. 코: 구토 냄새 감소. 손(장갑): 천 부드러움.',
   success_rate_vi:'Xử lý sớm + đúng bảo hộ (găng nitrile): 60–80%. Đã sấy / lụa: thấp — báo trước.',
   success_rate_ko:'즉시+보호구(장갑): 60–80%. 건조·실크: 낮음.',
   refuse_when_vi:'Không găng → dừng. Lụa/len vết lớn → giặt khô. Khách đòi 100% sau sấy → từ chối / báo trước.',
   refuse_when_ko:'장갑 없이 작업 금지. 실크·울 대량 → 드라이. 건조 후 100% 요구 → 사전 고지 거절.'},
  {id:'S_URINE',
   why_vi:'[Tại sao] Nước tiểu tươi = urea + muối (dễ). KHÔ = vi khuẩn → amoniac + uric acid (khó tan nước thường) → mùi khai + vàng. BẮT BUỘC găng. CẤM amoniac (cộng mùi). CẤM trộn Javel với amoniac (khí độc). THỨ TỰ: thấm/xả lạnh → enzyme phân giải đạm (phá uric) → giấm trung hòa → bột tẩy oxy nếu vàng trên đồ trắng.',
   fresh_path_vi:'(1) Găng tay. (2) Thấm hút tối đa — lực nhẹ, ngoài→trong, không chà. (3) Xả LẠNH mặt trái. (4) Ngâm enzyme phân giải đạm 30 phút. (5) Giặt lạnh/ấm nhẹ. (6) Kiểm tra mùi khi ướt + ánh sáng (vàng).',
   dried_path_vi:'(1) Ngâm lạnh 15 phút. (2) Enzyme phân giải đạm nồng độ cao 45–60 phút — bước QUAN TRỌNG (uric). (3) Giấm trắng 1:4 ~30 phút. (4) Tuỳ chọn baking soda khử mùi. (5) Giặt + bột tẩy oxy trên đồ trắng nếu còn vàng. (6) Phơi nắng. Đệm/thảm: xử lý cục bộ enzyme+giấm+baking soda — không giặt máy chung đồ sạch.',
   force_metaphor_vi:'Tươi: lực nhẹ (chỉ thấm). Khô: lực vừa + ngâm enzyme dài — không chà mạnh',
   force_metaphor_ko:'신선: 약하게 흡수만. 마른 후: 중간 + 효소 장침지 — 세게 문지르지 말 것',
   sense_check_vi:'Mũi: hết khai khi ướt. Mắt: không vàng. Tay: vải mềm.',
   sense_check_ko:'코: 젖은 상태에서도 암모니아 냄새 없음. 눈: 황변 없음. 손: 천 부드러움.',
   success_rate_vi:'Tươi: cao (~80%). Khô đệm/thảm: ~50% — báo trước.',
   success_rate_ko:'신선: 높음(~80%). 마른 매트리스: ~50% — 사전 고지.',
   refuse_when_vi:'CẤM amoniac / CẤM trộn Javel với amoniac. Không cam kết 100% mùi cũ trên nệm.',
   refuse_when_ko:'암모니아·염소(락스) 혼합 금지(가스 위험). 오래된 매트리스 냄새 100% 비보장.'},
  {id:'S_FECES',
   why_vi:'[Tại sao] Phân = đạm + vi khuẩn (nguy cơ cao) + sắc tố mật (bile) khó tẩy. Bảo hộ nặng: găng dày + khẩu trang + khu riêng + sát khuẩn bàn. CẤM trộn với đồ sạch. CẤM nóng lúc đầu. THỨ TỰ: cạo rắn → xả lạnh → enzyme phân giải đạm nồng độ cao → bột tẩy oxy cho màu mật → giặt nhiệt cao an toàn vải + nắng.',
   fresh_path_vi:'(1) Găng dày + khẩu trang; niêm bàn bẩn. (2) Cạo hết chất rắn vào túi — lực vừa, ngoài→trong, KHÔNG chà. (3) Xả LẠNH mạnh mặt trái. (4) Ngâm enzyme phân giải đạm nồng độ cao 45–60 phút. (5) Còn màu vàng-nâu: bột tẩy oxy 1–2 giờ (sau bước đạm). (6) Giặt nhiệt cao nhất an toàn vải. (7) Phơi nắng. Sát khuẩn dụng cụ + bàn.',
   dried_path_vi:'Ngâm enzyme qua đêm → bột oxy dài → cotton trắng có thể Javel pha loãng SAU bước đạm (test góc). Báo màu mật có thể còn nhẹ.',
   force_metaphor_vi:'Lực vừa: cạo sạch trước — chà khi ướt = lan sâu',
   force_metaphor_ko:'중간: 고체 먼저 제거 — 젖은 채 문지르면 번짐·심부 침투',
   sense_check_vi:'Mắt: hết rắn + màu nhạt. Mũi: hết. Sau: bàn/dụng cụ đã sát khuẩn.',
   sense_check_ko:'눈: 고체 없음·색 옅음. 코: 냄새 없음. 작업대·도구 소독 완료.',
   success_rate_vi:'Đạm sạch: tốt; màu mật: ~65% — báo trước.',
   success_rate_ko:'단백질 제거: 양호; 담즙 색소: ~65% — 사전 고지.',
   refuse_when_vi:'Lụa/len → giặt khô. Không bảo hộ → dừng. Không giặt chung đồ sạch.',
   refuse_when_ko:'실크·울 → 드라이. 보호구 없이 작업 금지. 청결 세탁물과 혼합 금지.'},
  {id:'S_BABY_FORMULA',
   why_vi:'[Tại sao] Sữa bột = casein/whey + mỡ thực vật + SẮT (iron) + lactose. Khó hơn sữa tươi. CẤM Javel (clo + sắt = vết CAM VĨNH VIỄN). CẤM nóng lúc đầu (khóa sắt+đạm). THỨ TỰ: cạo → xả lạnh → nước rửa chén (mỡ) → enzyme phân giải đạm → giấm nếu vàng/cam (sắt) → giặt ấm nhẹ. Phơi nắng tốt cho đồ bé.',
   fresh_path_vi:'(1) Cạo sữa đặc. (2) Xả LẠNH mặt trái. (3) Nước rửa chén 2–4 giọt, massage nhẹ (lực vừa). (4) Xả. (5) Ngâm enzyme phân giải đạm lạnh 30–45 phút. (6) Còn vàng/cam: giấm trắng ~15 phút trên vết. (7) Giặt 30–35℃. (8) Ánh sáng TRƯỚC sấy; phơi nắng OK.',
   dried_path_vi:'Ngâm enzyme dài → nước rửa chén → giấm cho sắt. VẪN CẤM Javel. Báo nếu đã giặt nóng/Javel: vết cam khó phục.',
   force_metaphor_vi:'Lực vừa: nước rửa chén trước, enzyme sau — mỡ thực vật bám dai',
   force_metaphor_ko:'중간: 주방세제(유분)→효소(단백질) 순서 — 분유 식물성 지방이 끈김',
   sense_check_vi:'Tay: hết dính/trơn. Mắt: hết trắng sữa + không cam. Mũi: hết chua.',
   sense_check_ko:'손: 끈적·미끄럼 없음. 눈: 분유 잔여·주황 기미 없음. 코: 신내 없음.',
   success_rate_vi:'Sớm + đúng thứ tự: cao. Đã Javel/nóng: sắt cam — thấp.',
   success_rate_ko:'즉시·정석 순서: 높음. 이미 B2·온수: 철분 주황 — 낮음.',
   refuse_when_vi:'Khách đã dùng Javel → báo vết cam có thể vĩnh viễn.',
...
UNWIND [
  {id:'S_ENGINE_OIL',
   why_vi:'[Tại sao] Dau dong co = hydrocarbon + carbon den — kho nhat nhom dau. THONG GIO bat buoc khi dung moi (D1). THU TU: cao dac → N3 hut (lap) → D1 xit + tham (giay duoi) → A1 cham mau con (test) → D3 giat nong neu cotton/poly. CAM say khi con nhon. Lua/len: khong buoc nong.',
  ...
  {id:'S_GUM',
   why_vi:'[Tại sao] Keo cao su = polymer dan — DONG LANH la chia khoa. Am = dinh lai. THU TU: tui nilon + tu dong 30-60 phut → be/cao ngay khi gion → A2 it neu can dau (CAM acetate/rayon) → D2 giat. Khong ui nong len keo.',
  ...
  {id:'S_CANDLE_WAX',
   why_vi:'[Tại sao] Sap nen = sap + mau (neu mau). DONG/pha vo sap TRUOC — roi UI thap qua giay tham hut sap. Sau do D2/D1 neu can dau. CAM ui truc tiep khong giay (lan mau).',
  ...
  {id:'S_DEODORANT',
   why_vi:'[Tại sao] Vet trang khu mui = tinh the muoi nhom (Al). A3 (giam) hoa tan Al3+. CAM B2 (chlorine) — vang VINH VIEN. Neu vang nach (mo+protein): B1/enzyme sau khi trang sach — khac protocol trang.',
  ...
  {id:'S_PERFUME',
   why_vi:'[Tại sao] Nuoc hoa = con + tinh dau + mau — de VANG vai trang theo thoi gian. Xu ly SOM. THU TU: xa/tham → A3 1:4 ngam → giat. Trang: theo doi vang; B1 phong neu can. CAM say khi con con/dau thom dam.',
...
SET s.why_vi = o.why_vi, s.fresh_path_vi = o.fresh_path_vi, s.dried_path_vi = o.dried_path_vi,
    ...
    s.refuse_when_vi = o.refuse_when_vi, s.refuse_when_ko = o.refuse_when_ko
RETURN count(s) AS updated""")
        _r(s, "Z10_paths_b4_b5_rich", """
UNWIND [
  {id:'S_BBQ_SAUCE',
   why_vi:'[Tại sao] Sot BBQ = duong/tomate mau + dau + protein (neu thit). 3 LOP: E1 protein → D2 dau → A3 tannin/mau → B1 trang neu can. CAM nong/say khi con vet — khoa mau+duong.',
   fresh_path_vi:'(1) Cao bot. (2) Xa LANH. (3) E1 ngam 20-30 phut. (4) D2 Cap2. (5) A3 1:4 ~15 phut. (6) Trang: B1. (7) Giat; anh sang TRUOC say.',
   dried_path_vi:'E1 dai → D2 → A3 → B1 trang. Bao duong kho de vang.',
   force_metaphor_vi:'Cap2: lop tung lop — khong gop 1 lan',
   force_metaphor_ko:'Cap2: 층별 처리 — 한 번에 섞어 쓰지 말 것',
   sense_check_vi:'Tay: het nhon/ngot. Mat: mau nhat. Mui: het BBQ.',
   sense_check_ko:'손: 기름·단맛 없음. 눈: 색소 감소. 코: BBQ 냄새 감소.',
   success_rate_vi:'Tuoi: cao. Da say: trung binh.',
   success_rate_ko:'신선: 높음. 건조 후: 중간.',
   refuse_when_vi:'Lua + B1. Khach doi 100% mau khoa → bao.',
   refuse_when_ko:'실크+산소표백 주의. 고착 색소 100% 비보장.'},
  {id:'S_MUSTARD',
   why_vi:'[Tại sao] Mu-ta-det = curcumin mau vang (UV nhay) + bot. THU TU: cao → paste baking soda → B1/oxy → PHOI NANG (UV tay mau). CAM say khi con vang.',
   fresh_path_vi:'(1) Cao. (2) Paste N1+nuoc Cap2 15-30 phut. (3) Xa. (4) B1 trang/cotton. (5) Phoi nang neu duoc. (6) Giat; kiem tra anh sang.',
   dried_path_vi:'N1 + B1 + UV lap. Bao mau curcumin dai.',
   force_metaphor_vi:'Cap2 paste — UV sau la dong minh',
   force_metaphor_ko:'Cap2 베이킹소다 페이스트 — 이후 햇빛이 아군',
   sense_check_vi:'Mat: vang nhat. Anh sang/UV: giam.',
   sense_check_ko:'눈: 노란기 감소. 햇빛/강광에서 확인.',
   success_rate_vi:'UV+oxy: tot. Lua: thap.',
   success_rate_ko:'UV+산소: 양호. 실크: 낮음.',
   refuse_when_vi:'Lua mau — test; khong cam ket trang 100%.',
   refuse_when_ko:'유색 실크 — 테스트; 100% 복원 비보장.'},
  {id:'S_NAIL_POLISH',
   why_vi:'[Tại sao] Sơn móng = nitrocellulose + màu + dung môi. Acetone / nước tẩy sơn móng không dầu: thấm từ MẶT TRÁI + giấy thấm dưới — CẤM chà lan. CẤM acetate / rayon / triacetate (tan sợi). Test góc. Thông gió + găng nitrile. Tỷ lệ thấp nếu đã khô lâu — báo trước.',
   fresh_path_vi:'(1) Xác nhận không phải acetate/rayon — nếu là → dừng/chuyển. (2) Test góc. (3) Giấy thấm dưới. (4) Acetone thấm khăn, chấm mặt trái lực nhẹ, đổi giấy khi nhuốm. (5) Lặp đến hết màu. (6) Xả + nước rửa chén. (7) Giặt. Ánh sáng TRƯỚC sấy.',
   dried_path_vi:'Lặp acetone nhiều lần. Báo không 100%. Acetate → dừng / chuyên.',
   force_metaphor_vi:'Lực nhẹ: thấm mặt trái — chà = lan màu sâu',
   force_metaphor_ko:'약하게: 안쪽에서 찍기 — 문지르면 색소 확산',
   sense_check_vi:'Mắt: màu giảm từng lần thấm. Tay: hết dính sơn.',
   sense_check_ko:'눈: 블롯마다 색소 감소. 손: 끈적임 없음.',
   success_rate_vi:'Tươi: trung bình. Khô lâu: thấp — báo trước.',
   success_rate_ko:'신선: 중간. 오래된: 낮음 — 사전 고지.',
   refuse_when_vi:'Acetate/rayon + acetone. In hoa mỏng. Không cam kết 100%.',
   refuse_when_ko:'아세테이트/레이온+아세톤 금지. 100% 비보장.'},
  {id:'S_GLUE',
   why_vi:'[Tại sao] Xac dinh loai keo TRUOC. 502/cyano: A2. Acrylic: N3/dau. Bang dinh: A1. Sai dung moi = te hon. Cao bot truoc. Test vai. CAM acetate + A2.',
   fresh_path_vi:'(1) Nhan loai keo. (2) Cao bot Cap1-2. (3) 502: A2 it Cap1. Acrylic: N3 10-15 phut. Bang: A1. (4) D2 + giat. (5) Test truoc moi buoc.',
   dried_path_vi:'Cung hon — dung moi dung loai + thoi gian dai. Bao de rach vai mong.',
   force_metaphor_vi:'Cap1 dung moi dung loai — khong doan mu',
   force_metaphor_ko:'Cap1: 접착제 종류별 용제 — 추측 사용 금지',
   sense_check_vi:'Tay: het dan. Mat: het manh keo.',
   sense_check_ko:'손: 끈적임 없음. 눈: 접착제 잔여 없음.',
   success_rate_vi:'Dung dung moi: tot. Sai loai: thap.',
   success_rate_ko:'용제 맞으면: 양호. 종류 틀리면: 낮음.',
   refuse_when_vi:'Acetate+A2. Vai mong dat tien → chuyen neu khong chac.',
   refuse_when_ko:'아세테이트+아세톤 금지. 고가 얇은 원단 불확실 시 전문.'},
  {id:'S_PAINT_LATEX',
   why_vi:'[Tại sao] Son nuoc (latex) UOT = de (xa lanh + cao + D2). KHO = kho — dung moi/test goc, nhieu lan. CAM say khi con son. Thong bao ty le thap khi da kho.',
   fresh_path_vi:'(1) Xu ly SOM khi uot. (2) Cao + xa LANH Cap2. (3) D2. (4) Giat. (5) Anh sang TRUOC say.',
   dried_path_vi:'Test goc dung moi nhe + tham lap. Bao thap; co the chuyen.',
   force_metaphor_vi:'Cap2 uot — kho thi Cap1 blot dung moi',
   force_metaphor_ko:'젖었을 때 Cap2; 마른 뒤 Cap1 용제 찍기',
   sense_check_vi:'Mat: het mang son. Tay: het ron son.',
   sense_check_ko:'눈: 페인트 막 없음. 손: 페인트감 없음.',
   success_rate_vi:'Uot: cao. Kho >24h: thap.',
   success_rate_ko:'젖은 상태: 높음. 24시간 이상 건조: 낮음.',
   refuse_when_vi:'Son dau/enamel → khac protocol / chuyen. Khong 100% son kho.',
   refuse_when_ko:'유성 페인트는 별도/전문. 마른 수성 100% 비보장.'},
  {id:'S_EGG',
   why_vi:'[Tại sao] Trung = protein (long trang) + mo (long do). CAM nong luc dau (nau protein). THU TU: cao → xa LANH → E1 → D2 (long do) → giat.',
   fresh_path_vi:'(1) Cao long do/trang. (2) Xa LANH. (3) E1 20-30 phut. (4) D2 neu con nhon. (5) Giat lanh/am nhe. (6) Anh sang TRUOC say.',
   dried_path_vi:'E1 ngam dai. Bao da say khoa protein.',
   force_metaphor_vi:'Cap1-2 cao + lanh — khong ui/nong',
   force_metaphor_ko:'Cap1–2 긁기+찬물 — 온수·다림질 금지',
   sense_check_vi:'Tay: het nhon long do. Mat: het mang trang.',
   sense_check_ko:'손: 노른자 미끄럼 없음. 눈: 흰자 막 없음.',
   success_rate_vi:'Lanh+E1: cao. Da nong: thap.',
   success_rate_ko:'찬물+효소: 높음. 이미 온수: 낮음.',
   refuse_when_vi:'Lua: E1 cam / S1. Da nau chin vao soi → bao.',
   refuse_when_ko:'실크: 효소 주의/전문. 이미 익어 고착 → 고지.'},
  {id:'S_MILK',
   why_vi:'[Tại sao] Sua = casein/whey + mo nhe. De chua o VN. CAM nong luc dau. THU TU: xa lanh → E1 → D2 neu mo → giat. Khac sua bot (sat) — khong B2 nhu cong thuc.',
   fresh_path_vi:'(1) Xa LANH. (2) E1 20-30 phut. (3) D2 neu nhon. (4) Giat. (5) Kiem tra mui chua TRUOC say.',
   dried_path_vi:'E1 dai + A3 neu mui chua. Bao sua chua kho.',
   force_metaphor_vi:'Cap1-2 lanh — protein de khoa',
   force_metaphor_ko:'Cap1–2 찬물 — 단백질 고착 주의',
   sense_check_vi:'Mui: het chua. Tay: het nhon. Mat: het trang sua.',
   sense_check_ko:'코: 신내 없음. 손: 미끄럼 없음. 눈: 우유 잔여 없음.',
   success_rate_vi:'Tuoi: cao. Chua kho: trung binh.',
   success_rate_ko:'신선: 높음. 신 우유 건조: 중간.',
   refuse_when_vi:'Nham sua bot/sat → xem S_BABY_FORMULA. Lua: nhe.',
   refuse_when_ko:'분유·철분 의심 → 분유 SOP. 실크: 약하게.'},
  {id:'S_CHOCOLATE',
   why_vi:'[Tại sao] Socola = cacao tannin + mo + duong + protein sua. THU TU: cao → xa lanh → E1 → D2 → A3/B1 trang. CAM nong/say khi con mo+mau.',
   fresh_path_vi:'(1) Cao. (2) Xa LANH. (3) E1 30 phut. (4) D2. (5) A3 neu mau; B1 trang. (6) Giat; anh sang TRUOC say.',
   dried_path_vi:'E1+D2 dai → B1 trang. Bao cacao dam kho.',
   force_metaphor_vi:'Cap2: mo truoc/sau protein dung thu tu',
   force_metaphor_ko:'Cap2: 지방·단백질 순서 지키기',
   sense_check_vi:'Tay: het nhon. Mat: nau nhat. Mui: het cacao.',
   sense_check_ko:'손: 미끄럼 없음. 눈: 갈색 감소. 코: 초코 냄새 감소.',
   success_rate_vi:'Tuoi: cao. Dam/kho: trung binh.',
   success_rate_ko:'신선: 높음. 진하고 마른: 중간.',
   refuse_when_vi:'Lua + B1. Khong 100% vet dam.',
   refuse_when_ko:'실크+산소 주의. 진한 얼룩 100% 비보장.'},
  {id:'S_GRASS',
   why_vi:'[Tại sao] Co xanh = chlorophyll + protein + tannin. THU TU: A1 (con) tham mau xanh → E1 → B1 trang. CAM say khoa xanh. Khac "ho tinh bot/pul ieem".',
   fresh_path_vi:'(1) Cao dat. (2) A1 blot Cap1 mat trai + giay. (3) E1 ngam. (4) B1 trang neu can. (5) Giat; anh sang TRUOC say.',
   dried_path_vi:'A1 lap → E1 → B1. Bao xanh khoa.',
   force_metaphor_vi:'Cap1 blot A1 — khong cha lan xanh',
   force_metaphor_ko:'Cap1 알코올 찍기 — 문지르면 초록 번짐',
   sense_check_vi:'Mat: xanh nhat. Anh sang: het vet.',
   sense_check_ko:'눈: 초록 감소. 강광: 잔존 없음.',
   success_rate_vi:'Tuoi+A1: tot. Da say: trung binh.',
   success_rate_ko:'신선+알코올: 양호. 건조 후: 중간.',
   refuse_when_vi:'Lua: A1 test. Khong nham voi starch transfer.',
   refuse_when_ko:'실크: 알코올 테스트. 풀 이염과 혼동 금지.'},
  {id:'S_MUD',
   why_vi:'[Tại sao] Bun/dat = hat dat + tannin. VANG QUY TAC: de KHO → chai bot → roi giat. Uot cha = day bun vao soi. Khac laterite do (X2).',
   fresh_path_vi:'(1) De kho neu uot. (2) Chai/phui bot Cap1-2 ngoai troi. (3) Xa/giat D3. (4) Con mau: A3/B1. (5) Anh sang.',
   dried_path_vi:'Chai ky → giat. Mau dat: A3. Dat DO → S_LATERITE.',
   force_metaphor_vi:'Cap1 chai kho TRUOC — cam cha uot',
   force_metaphor_ko:'Cap1: 마른 뒤 털기 — 젖은 채 문지르기 금지',
   sense_check_vi:'Mat: het hat dat. Tay: het ran.',
   sense_check_ko:'눈: 흙 입자 없음. 손: 거칠음 없음.',
   success_rate_vi:'Kho+chai: cao. Cha uot: thap.',
   success_rate_ko:'건조 후 브러시: 높음. 젖은 채 문지름: 낮음.',
   refuse_when_vi:'Dat do laterite → protocol X2. Lua: chai nhe.',
   refuse_when_ko:'적토(라테라이트) → X2 SOP. 실크: 약하게.'},
  {id:'S_SWEAT_FRESH',
   why_vi:'[Tại sao] Mo hoi tuoi = muoi + protein + vi khuan (mui). E1 + giat lanh/am. Khac VE O NACH VANG (S_SWEAT_YELLOW) / deodorant Al. CAM B2 len nach vang.',
   fresh_path_vi:'(1) Nhan: tuoi (mui) vs vang nach. (2) E1 ngam 30 phut. (3) Giat. (4) A3 neu mui. (5) Phoi thoang.',
   dried_path_vi:'E1 dai + A3. Neu vang nach → S_SWEAT_YELLOW protocol.',
   force_metaphor_vi:'Cap1-2 enzyme — khong che mui bang xa vai',
   force_metaphor_ko:'Cap1–2 효소 — 섬유유연제로 냄새 숨기지 말 것',
   sense_check_vi:'Mui: het khi uot. Mat: khong vang moi.',
   sense_check_ko:'코: 젖은 상태에서도 땀내 없음. 눈: 새 황변 없음.',
   success_rate_vi:'Tuoi: cao. Vang cu: xem yellow SOP.',
   success_rate_ko:'신선: 높음. 오래된 황변: 황변 SOP.',
   refuse_when_vi:'Vang nach + B2 → bao. Khong cam ket mui cu 100%.',
   refuse_when_ko:'겨드랑이 황변+염소 금지. 오래된 냄새 100% 비보장.'}
] AS o
MATCH (s:Stain {id:o.id})
SET s.why_vi = o.why_vi, s.fresh_path_vi = o.fresh_path_vi, s.dried_path_vi = o.dried_path_vi,
    s.tip = coalesce(o.why_vi, s.tip),
    ...
    s.refuse_when_vi = o.refuse_when_vi, s.refuse_when_ko = o.refuse_when_ko
RETURN count(s) AS updated""")
        _r(s, "Z11_paths_rail_c_stains", """
UNWIND [
  {id:'S_SUNSCREEN',
   why_vi:'[Tại sao] Kem chong nang VN = UV filter + dau silicon + ZnO. N3 (dau) hoa tan UV/silicon → D2 nhu hoa → giat. CAM B2 (vang VINH VIEN). Phoi nang ngan OK sau sach.',
   fresh_path_vi:'(1) Cao bot Zn. (2) N3/dau dua phu 15-20 phut. (3) D2 Cap3 + xa nong nhe neu vai cho. (4) Con vang: B1 (CAM B2). (5) Giat 40-60C neu cho. (6) Anh sang TRUOC say.',
   dried_path_vi:'N3+D2 lap. Da B2 → bao vang co the vinh vien.',
   force_metaphor_vi:'Cap2-3: dau truoc xa phong — cam Javel',
   force_metaphor_ko:'Cap2–3: 오일 흡착 후 세제 — 락스(B2) 금지',
   sense_check_vi:'Tay: het nhon kem. Mat: het trang Zn/vang moi.',
   sense_check_ko:'손: 크림 미끄럼 없음. 눈: 아연 흰자국·새 황변 없음.',
   success_rate_vi:'SOM dung thu tu: cao. Da B2: thap.',
   success_rate_ko:'즉시·정석: 높음. 이미 B2: 낮음.',
   refuse_when_vi:'Khach da Javel → bao. Lua: N3/D2 nhe test.',
   refuse_when_ko:'이미 락스 → 황변 고지. 실크: 약하게 테스트.'},
  {id:'S_TAR',
   why_vi:'[Tại sao] Nhua duong = dau KHO NHAT. THONG GIO. Dong cao → D1 + giay tham 6-10 lan → D2 → giat. Bao kiên nhẫn + ty le thap.',
   fresh_path_vi:'(1) PPE + thong gio. (2) Dong 15 phut, cao Cap2. (3) D1 + giay duoi, doi 5 phut, tham; lap 6-10. (4) D2 Cap3 cotton/denim. (5) Giat 40-50C neu cho. (6) Anh sang TRUOC say.',
   dried_path_vi:'Lap D1 nhieu. Lua/len: luc 1 / chuyen.',
   force_metaphor_vi:'Cap2 blot D1 lap — khong cha lan',
   force_metaphor_ko:'Cap2: 용제 찍어 반복 — 문질러 번지지 말 것',
   sense_check_vi:'Mat: nhua giam moi lan. Tay: het dinh nhua.',
   sense_check_ko:'눈: 타르가 회차마다 감소. 손: 끈적임 없음.',
   success_rate_vi:'Lap nhieu: trung binh. Mot lan: thap.',
   success_rate_ko:'다회 반복: 중간. 1회: 낮음.',
   refuse_when_vi:'Khong thong gio. Vai mong dat → chuyen. Khong 100%.',
   refuse_when_ko:'환기 없이 용제 금지. 고가 얇은 원단 → 전문. 100% 비보장.'},
  {id:'S_MASCARA',
   why_vi:'[Tại sao] Masca = carbon black + sap/dau. Thuong: A1. Chong nuoc: D1. GIAY THAM duoi. KHONG cha. CAM B2 (xam vinh vien).',
   fresh_path_vi:'(1) Nhan thuong vs chong nuoc. (2) Giay duoi. (3) A1 hoac D1 blot Cap1 mat trai, doi giay. (4) D2. (5) Trang con: B1 (CAM B2). (6) Giat; anh sang.',
   dried_path_vi:'Lap blot. Bao carbon kho het 100%.',
   force_metaphor_vi:'Cap1 blot — cha = lan den',
   force_metaphor_ko:'Cap1 찍기 — 문지르면 검정 번짐',
   sense_check_vi:'Mat: den giam tren giay. Tay: het dan.',
   sense_check_ko:'눈: 종이에 검정이 옮아 감소. 손: 끈적임 없음.',
   success_rate_vi:'Tuoi: trung binh-cao. Chong nuoc kho: thap.',
   success_rate_ko:'일반·신선: 중~높음. 워터프루프 건조: 낮음.',
   refuse_when_vi:'B2 cam. Lua: test A1.',
   refuse_when_ko:'염소(B2) 금지. 실크: 알코올 테스트.'},
  {id:'S_HAIR_DYE',
   why_vi:'[Tại sao] Thuoc nhuom toc = pigment oxi hoa — SOM xa lanh. A1/B1 can than. Test. Da kho mau: ty le thap — bao truoc.',
   fresh_path_vi:'(1) Xa LANH ngay Cap1. (2) A1 blot neu mau. (3) B1 trang/cotton (test). (4) Giat. (5) Anh sang TRUOC say. CAM say khi con mau.',
   dried_path_vi:'A1+B1 lap. Bao kho phuc 100%.',
   force_metaphor_vi:'Cap1 blot — khong cha lan nhuom',
   force_metaphor_ko:'Cap1 찍기 — 문지르면 염료 확산',
   sense_check_vi:'Mat: mau nhat tung buoc. Test goc OK.',
   sense_check_ko:'눈: 회차마다 색소 감소. 구석 테스트 OK.',
   success_rate_vi:'SOM: trung binh. Da say: thap.',
   success_rate_ko:'즉시: 중간. 건조 후: 낮음.',
   refuse_when_vi:'Vai mau nhay / lua — test fail thi dung. Khong 100%.',
   refuse_when_ko:'예민 유색·실크 — 테스트 실패 시 중단. 100% 비보장.'}
] AS o
MATCH (s:Stain {id:o.id})
SET s.why_vi = o.why_vi, s.fresh_path_vi = o.fresh_path_vi, s.dried_path_vi = o.dried_path_vi,
    s.tip = coalesce(o.why_vi, s.tip),
...
UNWIND [
  ...
  {id:'I_NECKTIE',name:'Necktie / silk or poly tie',name_vi:'Ca vat / caravat (lua hoac poly)',name_ko:'넥타이(실크·폴리)',fabric_id:'F4',
   ...
   why_vi:'[Tại sao] Ca vat = giu HINH. Wet wash/ngam = bien dang. Vet: tham Cap1 NGOAI→TRONG + khan lot duoi; D2/S1 pha loang CHAM cuc bo. CAM chai mem/cung (sai sol → ultra). CAM binh xit ngap / chau ngam. Nhao: steamer DUNG, khong ep manh.',
  ...
  {id:'I_GOLF_HAT',name:'Golf / sports cap',name_vi:'Mu golf / mu luoi trai',name_ko:'골프모자·캡',fabric_id:'F2',
   ...
   why_vi:'[Tại sao] Mu cau truc = giu form. New Era/cap: CHI spot-clean vanh + panel — CAM ngam toan bo / may (vanh vo). Chat NHE/trung tinh + chai mem. A3 loang chi vanh neu vang mo hoi (test mau). Nhet bat/bong giu form khi kho.',
  ...
  {id:'I_HAT_CAP',name:'Baseball / fashion cap (structured or soft)',name_vi:'Mu luoi trai / baseball cap',name_ko:'야구모자·캡·일반 모자',fabric_id:'F2',
   ...
   why_vi:'[Tại sao] Mo hoi vanh = dau+muoi+protein. Thu tu: spot vanh → giu form khi kho. CAM nhiet (vanh mem/vo). Len: lanh cuc, khong cha. Tham khao New Era care: mild detergent, air dry natural position.',
  ...
  {id:'I_DRESS_SHIRT',name:'Dress shirt / white business shirt care',name_vi:'Ao so mi cong so (cham soc thuong)',name_ko:'와이셔츠·드레스셔츠(일반 세탁·관리)',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Ao so mi cotton/cotton-pha = giat thuong + pretreat co/tay ao. KHONG mac dinh = SOP tay vang. Cam Javel thuong xuyen (yeu soi + vang protein). Co/tay ban: enzyme/D2 len CHO KHO truoc. Ui: co → manchette → tay → than. Ho tinh bot (tuy chon) SAU khi sach.',
  ...
  {id:'I_CURTAIN_FABRIC',name:'Fabric window curtain',name_vi:'Rem vai thuong (cua so)',name_ko:'일반 커튼·패브릭 커튼',fabric_id:'F2',
   ...
   why_vi:'[Tại sao] Rem vai = rut + moc (VN am). Giat tinh te 30C, D2/D3 it, vat ngan. Treo UOT de trong luc keo phang. Chu ky: 1 thang (phong), 2 tuan (tam). CAM Javel mau.',
  ...
  {id:'I_CURTAIN_URETHANE',name:'Urethane / vinyl / PU coated curtain',name_vi:'Rem phu urethane / vinyl / PU (tam tam)',name_ko:'우레탄·비닐·PU 코팅 커튼·샤워커튼',fabric_id:'F2',
   ...
   why_vi:'[Tại sao] Lop phu PU/vinyl: may nhieu = boc/lot (IFI/coated fabric). Uu tien: xa phong TRUNG TINH + nuoc am + mien fot/sol mem. CAM dung moi manh/A2. Neu may (chi khi nhan cho): tinh te, <=40C, them khan can bang, CAM xa vai, CAM say — treo kho. Phenol CAM tren urethane; ammonia pha OK hon tren PU (medical fabric guides).',
  ...
  {id:'I_DUVET_GOOSE',name:'Goose / duck down duvet',name_vi:'Chan long ngong/vit (down)',name_ko:'구스이불·거위털·다운 이불',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Down = giu dau tu nhien. CAM dry-clean PERC (hut dau, gion). Uu tien giat nuoc: may LON front-load, nuoc LANH/<=30C, chat down-wash / trung tinh IT, THEM xa. Say thap + bong tennis/dryer ball, dung-fluff 2-3h — KHO 100% (moc). Feathered Friends/FabricCare: mild, no bleach/softener.',
  ...
  {id:'I_DUVET_COTTON',name:'Cotton batting / synthetic fill comforter',name_vi:'Chan bong / chan poly (khong long)',name_ko:'솜이불·폴리이불·일반 충전 이불',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Bong/poly ben hon down nhung de von neu thieu xa/say. May lon, tinh te/bulky, 30-40C, D2/D3 IT, THEM xa (bot ton = cung). Say thap/vua + bong tennis; dung-fluff. CAM xa vai. Acme/Bedsure: drum phai du cho tumbling.',
  ...
  {id:'I_BED_SHEET',name:'Bed sheet / fitted sheet',name_vi:'Ga giuong / drap',name_ko:'침대 시트·매트리스커버',fabric_id:'F1',
   precheck_vi:'Tach trang vs mau. Lot/bo ga rieng. Doc nhan. Vet huu co → PPE.',
   why_vi:'[Tại sao] Ga = 60C diet mite + UV nang VN. Trang: B1 neu vang. Mau: khong Javel. Phoi nang = sat khuan.',
   fresh_path_vi:'(1) Phui/hut bui. (2) Spotting vet. (3) May D3 ~60C neu cotton cho (mau: 40C). (4) B1 trang neu vang. (5) Phoi nang neu duoc. (6) Kho han moi dap.',
   dried_path_vi:'Mui/moc: A3 ngam roi 60C. Vet protein: E1 lanh truoc.',
   motion_vi:'Luc 0 may; spotting Cap2',
   water_temp_vi:'Trang cotton: ~60C. Mau/nhay: theo nhan ~40C',
   aftercare_vi:'Phoi nang/thoang. Thay ga dinh ky. Tach do nguoi lon vs be.'},
  {id:'I_TOWEL',name:'Bath towel / washcloth',name_vi:'Khan tam / khan mat',name_ko:'수건·목욕타월',fabric_id:'F1',
   precheck_vi:'Tach mau dam. Khan mui: A3 truoc. CAM xa vai (mat tham).',
   why_vi:'[Tại sao] Khan 60C diet khuan. CAM softener/xa vai = mat tham. Say vua/cao = xop. VN: bat dau kho <4h.',
   fresh_path_vi:'(1) Mui: A3 250ml/5L ngam 30 phut. (2) Giat 60C + D3. (3) CAM xa vai. (4) Say vua hoac phoi + ru manh. (5) Kho han.',
   dried_path_vi:'Mui dai: lap A3+60C. Cung: bot it + xa them.',
   motion_vi:'Luc 0 may',
   water_temp_vi:'~60C (cotton); theo nhan',
   aftercare_vi:'Say/phoi thoang. Khong qua nhieu bot.'},
  {id:'I_BABY_WEAR',name:'Baby / infant clothing',name_vi:'Do tre so sinh / em be',name_ko:'아기·유아복',fabric_id:'F1',
   precheck_vi:'Giat RIENG nguoi lon. Bot KHONG huong / khong OBA. Vet sua/phan → cold protein SOP.',
   why_vi:'[Tại sao] Da be = it kich ung. Bot khong muoi. 60C ve sinh / 40C tinh te. THEM xa. Vet: LUON lanh truoc (CAM nong khoa protein).',
   fresh_path_vi:'(1) Tach lo. (2) Spotting: sua/phan E1 lanh; nuoc tieu A3. (3) Giat 40-60C bot baby. (4) Extra rinse. (5) Kho <4h VN.',
   dried_path_vi:'Mui: A3+xa them. Vet khoa: bao; xem S_BABY_FORMULA / S_URINE.',
   motion_vi:'Luc 1-2 spotting nhe',
   water_temp_vi:'Vet: lanh. Giat: 40-60C theo vai',
   aftercare_vi:'Kho han. Khong xa vai manh muoi.'},
  {id:'I_SWIMWEAR',name:'Swimwear / spandex suit',name_vi:'Do boi / do the thao stretch',name_ko:'수영복·스판덱스',fabric_id:'F2',
   precheck_vi:'Xa lanh NGAY sau boi (trung hoa clo/muoi). CAM say may / nang gay.',
   why_vi:'[Tại sao] Clo + nhiet = hong spandex. Tay: D3 it, lanh, vo nhe. May: tui luoi tinh te lanh. Ep khan — CAM vat xoan. Phoi bong mat.',
   fresh_path_vi:'(1) Xa lanh 5 phut ngay. (2) Tay/D3 it hoac may tui luoi lanh. (3) Ep khan. (4) Phoi bong mat. (5) CAM dryer.',
   dried_path_vi:'Da say/nang: bao dan hoi giam — khong phuc hoi 100%.',
   motion_vi:'Luc 1 — vo nhe, khong xoan',
   water_temp_vi:'LANH only',
   aftercare_vi:'Bong mat. Khong la/say nong.'},
  {id:'I_ODOR_SMOKE',name:'Cigarette / smoke odor on textiles',name_vi:'Mui thuoc la / khoi tren vai',name_ko:'담배·연기 냄새 제거(섬유)',fabric_id:'F1',
   precheck_vi:'Chup anh. Tach do. Mui vo hinh — dung MUI kiem tra khi uot.',
   why_vi:'[Tại sao] Nicotine+tar bam soi. A3 1:4 → N1 hut → giat nong an toan → nang+gio. Khong xit nuoc hoa che mui.',
   fresh_path_vi:'(1) A3 1:4 ngam 30-60 phut. (2) N1 rac 1-2h. (3) Giat D2/D3 nhiet cao nhat an toan. (4) Phoi nang+gio >=4h. (5) Mui khi uot con → lap.',
  ...
  {id:'I_CARE_LABEL',name:'Care label symbols decoder',name_vi:'Doc ky hieu giat (nhan)',name_ko:'케어라벨·세탁표시 기호 해석',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Nhan = hop dong voi vai. (1) Chau nuoc=giat: so=max C; tay=chi tay; X=CAM nuoc. Gach chan=nhe/rat nhe. (2) Tam giac=tay: rong=B1/B2 OK; gach cheo=chi B1; X=CAM tay. (3) Vuong=say: cham=nhiet; X vong=CAM may say. (4) Ban ui=cham nhiet; X=CAM ui. (5) Vong tron=dry-clean: P/F/W; X=CAM dry-clean.',
  ...
  {id:'I_DRY_VS_WET',name:'Dry-clean vs wet-clean decision',name_vi:'Dry-clean vs giat uot — quyet dinh',name_ko:'드라이클리닝 vs 물세탁 판단',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Uu tien Nhan. X chau / P-F dry → chuyen dry-clean. Wet OK: cotton/poly nhan cho, towel, sheet. TU CHOI/chuyen: fur that, leather ungu, silk nhuan mau dat, suit canvas phuc tap, xuong long PERC (down uu tien wet mild). Nghi ngo → bao thap + chuyen.',
  ...
  {id:'I_INTAKE_SCRIPT',name:'Store intake / check-in scripts',name_vi:'Script tiep nhan / check-in',name_ko:'접수·체크인 스크립트',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Anh + chu ky = bao ve phap ly. Bat buoc: (1) chao (2) dem+ghi vet/hong san (3) CHUP tong+close-up (4) bao gia+thoi gian (5) phieu ky. Vet kho: thong bao truoc + dong y van ban. Khong anh khi nhan = de thua kien.',
  ...
  {id:'I_CLAIM_SCRIPT',name:'Claim / dispute resolution script',name_vi:'Script khieu nai / tranh chap',name_ko:'클레임·분쟁 대응 스크립트',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Anh+phieu = bang chung. Doi chieu → loi shop: xin loi+phuong an / hu san: dua anh.',
  ...
  {id:'I_PRICING_SCRIPT',name:'Pricing / quote script',name_vi:'Script bao gia',name_ko:'요금·가격 안내 스크립트',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Gia = ro + tu tin. Xem bang gia. Than dat → giai thich gia tri.',
  ...
  {id:'I_QUIZ_FABRIC',name:'Fabric safety quiz (staff)',name_vi:'Quiz vai an toan (nhan vien)',name_ko:'원단 안전 퀴즈(직원)',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Lua/len/acetate/tay.',
  ...
  {id:'I_WATER_HARDNESS',name:'Hard water / VN tap water correction',name_vi:'Nuoc cung / dieu chinh nuoc may VN',name_ko:'경수·베트남 수돗물 보정',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Nuoc cung (Ca/Mg) = bot giat kem hieu luc + can may + vai cang. Bu: (1) tang bot vua / dung chat cho nuoc cung (2) THEM xa (3) A3 1:4 xa cuoi tuy chon (4) ve sinh may dinh ky. Khong doi bang them xa vai (mat tham khan).',
  ...
  {id:'I_MACHINE_PROFILE',name:'Washer dryer program profiles',name_vi:'Cai may giat / may say theo vai',name_ko:'세탁기·건조기 코스 설정',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Giat: Cotton thuong; Tinh te=len/lua/mong; 60C+=khan/ga/be; The thao=mui. Bot IT + dung lieu. Say: Cao=khan; Vua=cotton; Thap=poly; CAM say len/lua/spandex/ao dai/da. VN: phoi bong mat + quat thuong du (<4h).',
  ...
  {id:'I_SORT',name:'Laundry sorting / load separation',name_vi:'Phan loai do giat (tach mau)',name_ko:'세탁물 분류·분리 세탁',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Sai phan loai = loang mau + mui + cheo nhiem. Phan loai truoc xu ly vet.',
  ...
  {id:'I_RINSE',name:'Rinse / extra rinse education',name_vi:'Xa / xa them (het bot)',name_ko:'헹굼·추가 헹굼 교육',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Xa kem = bot du + vang/cang. Down/chan/do be BAT BUOC xa them. Khong thay bang softener.',
  ...
  {id:'I_QC_HANDOVER',name:'QC checklist + customer handover',name_vi:'QC + ban giao khach',name_ko:'QC·출고 점검·고객 인도',fabric_id:'F1',
   ...
   why_vi:'[Tại sao] Thieu QC = khieu nai. Con vet ma say = khoa. Bao gioi han luc giao.',
...
FOREACH (_ IN CASE WHEN i.id IN ['I_CURTAIN_FABRIC','I_DUVET_COTTON','I_BED_SHEET','I_TOWEL','I_BABY_WEAR'] THEN [1] ELSE [] END |
  MERGE (i)-[:USES_TOOL]->(soft) MERGE (i)-[:USES_TOOL]->(cloth) MERGE (i)-[:USES_TOOL]->(mesh)
  MERGE (i)-[:USES_CHEMICAL]->(d2) MERGE (i)-[:USES_CHEMICAL]->(d3))
FOREACH (_ IN CASE WHEN i.id IN ['I_BED_SHEET','I_TOWEL'] THEN [1] ELSE [] END |
  MERGE (i)-[:USES_CHEMICAL]->(b1) MERGE (i)-[:USES_CHEMICAL]->(a3))
FOREACH (_ IN CASE WHEN i.id = 'I_BABY_WEAR' THEN [1] ELSE [] END |
  MERGE (i)-[:USES_CHEMICAL]->(e1) MERGE (i)-[:USES_CHEMICAL]->(a3))
FOREACH (_ IN CASE WHEN i.id = 'I_SWIMWEAR' THEN [1] ELSE [] END |
  MERGE (i)-[:USES_TOOL]->(cloth) MERGE (i)-[:USES_TOOL]->(mesh) MERGE (i)-[:USES_CHEMICAL]->(d2))
...
UNWIND [
  {id:'S_BUTTER',
   why_vi:'[Tại sao] Bo = mo dong vat (solid fat) — giong dau an nhung cung o nhiet phong. THU TU: N3 (bot ngo/tinh bot) hut 10-15 phut → chai nhe → D2 massage Cap2 → giat am. CAM say khi con nhon (khoa mo). Khong nuoc nong truoc khi hut (lan mo).',
  ...
  {id:'S_SHOE_POLISH',
   why_vi:'[Tại sao] Xi giay = sap/dau + PIGMENT mau (den/nau) — kho vi mau. THONG GIO khi dung moi. THU TU: cao bot → D1 (dung moi) blot mat trai + giay tham → D2 → A1 neu mau con (test) → B1 trang neu can. CAM cha lan pigment. Acetate/rayon: CAM A2.',
  ...
  {id:'S_GUM',
   ...
   refuse_when_ko:'아세테이트/레이온+아세톤 금지. 얇은 원단 긁힘 위험 고지.'},
  {id:'S_BUBBLE_TEA',
   why_vi:'[Tại sao] Tra sua tran chau = 4 lop: tannin tra + protein sua + tinh bot san (tran chau) + duong. THU TU: cao tran chau → xa LANH → enzyme (protein+tinh bot) → D2 (mo sua) → A3 (tannin) → B1 neu trang. CAM nuoc nong dau (khoa protein+mau). Bo enzyme = kem hieu qua.',
...
""",
        ...
        try:
            ...
            from education_vn_specialty_v43 import vn_specialty_stain_seed_rows_v43 as _vn_v43
