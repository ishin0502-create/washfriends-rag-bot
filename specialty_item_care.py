...
def apply_specialty_item_education(graph: dict, entities: Optional[dict] = None) -> dict:
    ...
    if real_stain and item_id not in {"I_MACHINE_PROFILE", "I_DRY_VS_WET"}:
        # Stain+item: keep stain SOP; hotel/bio notes only for towel/sheet as tip addendum
...
def _goose_duvet() -> dict[str, str]:
    return {
        ...
        "why_ko": (
            ...
            "소량 중성/다운세제 + 추가헹굼 + 저온건조+테니스볼로 뭉침 풀기. "
...
def _down_jacket() -> dict[str, str]:
    return {
        ...
        "dried_path_ko": "미건조 냄새: 재건조+볼. 로프트 50% 미만: 털 손상 가능 — 고객 고지.",
        "motion_ko": "Cap0–1. 세탁기 섬세. 비틀어 짜기 금지.",
        "water_temp_ko": "찬물 / ≤30℃. 건조 저온.",
        "aftercare_ko": "완전 건조 후 보관. 통풍. 건조 전 강광 잔여 확인.",
        "sense_check_ko": "손: 배플 가운데 건조·부피 회복. 코: 곰팡이·세제 냄새 없음.",
        "success_rate_ko": "전면투입+다운세제+완전건조: 높음. 미건조: 곰팡이 확정 위험.",
...
def _faux_leather() -> dict[str, str]:
    return {
        "precheck_ko": (
            "인조가죽(PU·PVC·레자·비건레더): 진가죽과 구분. "
            "표면이 균일·코팅감·가죽이 아닌 천 뒷면이면 인조. 라벨·고가·박리 시작이면 사진·거절권."
        ),
        "why_ko": (
            "[왜 이 순서] 인조가죽=플라스틱 코팅+원단. "
            "세탁기·통담금·고온·강한 용제·가죽크림(진가죽용) 강제는 코팅 갈라짐·박리. "
            "국소 중성·미지근한 천 + 완전 건조. 진가죽 L1/L2 경로와 섞지 말 것."
        ),
        "fresh_path_ko": (
            "(1)진가죽/스웨이드/인조 구분. 인조 확정. "
            "(2)마른 천으로 먼지. "
            "(3)중성세제(D2) 극소량 희석액을 흰 천에 묻혀 Cap1 국소 — 흠뻑·통담금·세탁기 비권장. "
            "(4)물기만 남은 천으로 잔여 → 그늘 완전 건조(형태 유지). "
            "(5)광택용 인조가죽 전용 케어(라벨 허용)만. 진가죽 크림·밍크오일·아세톤·알코올 과다 금지. "
            "(6)갈라짐·박리·고가 → 전문/거절."
        ),
        "dried_path_ko": "이미 세탁기 돌린 인조: 추가 강처리 중단, 코팅 손상 고지.",
        "motion_ko": "Cap1만 — 찍기·닦기. 세게 문지르기 금지.",
        "water_temp_ko": "미지근·최소. 온수·세탁기·건조기 금지(기본).",
        "aftercare_ko": "통풍 건조. 직사광선·히터·접어 장기보관(코팅 주름) 주의.",
        "sense_check_ko": "눈: 코팅 갈라짐·박리. 손: 끈적·잔여 세제.",
        "success_rate_ko": "국소 오염: 양호. 이미 박리·세탁기 후: 낮음.",
        "refuse_when_ko": "세탁기·드라이 강요, 진가죽 크림 강제, 박리 진행 → 거절/고지.",
    }
...
def _sneaker_white() -> dict[str, str]:
    return {
        "precheck_ko": (
            ...
            "산화·황변은 100% 복원 비보장 — 사전 고지. 끈은 빼서 별도."
        ),
        "why_ko": (
        ...
        "fresh_path_ko": (
            "(1)끈·깔창 분리. 마른 흙 털기. "
            "(2)흰 고무·옆면: 중성세제+베이킹소다 페이스트, 경질 솔(고무만) Cap2 한 방향. "
            "흰 천 패널: 연질 솔·중성만. "
            "(3)충분히 헹굼(잔여 세제=재황변). "
            "(4)흰 끈: 별도 찬물 담금 15–30분+중성 → 망세탁 약하게 → 헹굼. "
            "(5)그늘·통풍 건조, 신문지·키친타월로 형태. 고온건조기·직사광선 금지. "
            "(6)남아 있는 누런기: 1회 재시도. 산소표백은 흰 천·끈만 테스트 후. 고무는 신중. "
            "(7)100% 하얗게 약속 금지."
        ),
        "dried_path_ko": "오래된 산화 황변: 성공률 중간↓. 고객에 한계 고지. 강한 락스·사포 과다 금지.",
        "motion_ko": "고무 Cap2 경질 솔. 메쉬·갑피 Cap1–2 연질 — 솔 혼용 금지.",
        "water_temp_ko": "찬물~30℃. 온수·고온건조 금지.",
        "aftercare_ko": "완전 건조 후 착용. 그늘 보관. 재오염 시 바로 국소.",
        "sense_check_ko": "눈: 누런기·잔여. 손: 미끄럼(세제 잔여) 없음.",
        "success_rate_ko": "표면 때: 양호. 산화 황변: 중간~낮음 — 고지.",
        "refuse_when_ko": "100% 복원·락스 범벅·고온건조 요구 → 거절.",
    }
...
def _linen_garment(*, finish: bool = False) -> dict[str, str]:
    finish_bit = (
        ...
        if finish
        else " 마무리: 약간 촉촉할 때 다림질/스팀(고온)."
    )
    return {
        "precheck_ko": (
            "마(린넨) 의류: 라벨. 수축 3–8% 가능 — 고객 사전 고지. "
        ...
        "fresh_path_ko": (
            "(1)라벨·색이염 테스트. 수축 고지. "
            "(2)얼룩: 찬물 블롯 → 중성/일반세제 국소(단백질·탄닌은 해당 얼룩 SOP). "
        ...
        "aftercare_ko": "걸이 보관. 구김은 특성 — 스팀/에어드레서로 완화.",
...
def _finishing_matrix() -> dict[str, str]:
    return {
        ...
        "fresh_path_ko": (
            ...
            "(5)가죽·스웨이드·모피: 일반 다리미/에어드레서 고온 금지. "
...
def _suit_finishing(*, summer: bool = False) -> dict[str, str]:
    ...
    return {
        ...
        "fresh_path_ko": (
            ...
            "바지: 칼주름은 천 대고 신중 또는 전문 프레스. "
            "(3)에어드레서: 냄새·가벼운 구김 OK. 완벽한 칼주름·행사 직전은 수동 스팀 병행. "
            "(4)넓은 나무 옷걸이에 식히기. "
        ...
        "success_rate_ko": "스팀 피니싱: 높음. 가정기 세탁 후: 낮음.",
...
def _dress_shirt_finishing() -> dict[str, str]:
    return {
        ...
        "fresh_path_ko": (
            ...
            "(4)에어드레서: 일상 구김·보관용. 납품용 칼주름은 프레스/수동. "
            "(5)걸거나 바로 포장. 접으면 풀·주름 효과↓."
        ),
        "dried_path_ko": "이미 완전 건조면 분무 후 다림.",
        "motion_ko": "Cap2–3. 깃·커프는 또렷하게.",
        "water_temp_ko": "세탁은 라벨. 다림은 면 고온대.",
        "aftercare_ko": "걸이 또는 납품 접기 표준 준수.",
        "sense_check_ko": "눈: 깃·주름선·광택 손상.",
        "success_rate_ko": "순서 다림질: 높음.",
        "refuse_when_ko": "잔여 얼룩 상태 다림 → 거절.",
    }
